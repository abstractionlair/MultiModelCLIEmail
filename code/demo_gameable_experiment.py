"""
demo_gameable_experiment.py

Phase 1 Experiment: Demonstrate evaluator gaming with TriggerWordEvaluator.

Tests if QD search can exploit a deliberately gameable evaluator.

Condition A: Vulnerable ensemble (TriggerWord + mock evaluators)
Expected: Archive evolves toward keyword-stuffed outputs
"""
...
try:
    from enhanced_mutator import EnhancedDocstringMutator  # optional
except Exception:
    EnhancedDocstringMutator = None  # type: ignore
...
def run_condition_a(
    ...
    output_file: str = "experiment_results_condition_a.json"
):
    """Run Condition A: Vulnerable ensemble experiment."""
    
    print("=" * 70)
    print("CONDITION A: Vulnerable Ensemble")
    print("Testing if search exploits TriggerWordEvaluator")
    print("=" * 70)
    print()
    ...
    # Set up evaluators (vulnerable ensemble)
    print("Setting up VULNERABLE ensemble:")
    evaluators = [
        ...
        KeywordPreferenceEvaluator(
            "keyword-eval",
            keywords=["function", "returns", "calculate", "fibonacci"]
        ),
    ]
    judge_pool = JudgePool(evaluators, default_n_judges=3)
    print(f"  - TriggerWordEvaluator (10x weight on impressive words)")
    print(f"  - LengthPreferenceEvaluator (prefers longer)")
    print(f"  - KeywordPreferenceEvaluator (domain keywords)")
    print()
    ...
    use_enhanced = os.getenv("USE_ENHANCED_MUTATOR", "0") == "1"
    mutator = (
        EnhancedDocstringMutator() if (use_enhanced and EnhancedDocstringMutator)
        else DocstringMutator()
    )
    ...
    results = {
        'condition': 'A_vulnerable',
        'timestamp': datetime.now().isoformat(),
        'parameters': {
            'iterations': num_iterations,
            'evaluators': ['TriggerWord', 'Length', 'Keyword'],
            'seed': 42
        },
        'archive': {
...
if __name__ == '__main__':
    ...
    output = sys.argv[2] if len(sys.argv) > 2 else "experiment_results_condition_a.json"
    
    random.seed(42)
    results = run_condition_a(num_iterations=n_iter, output_file=output)
