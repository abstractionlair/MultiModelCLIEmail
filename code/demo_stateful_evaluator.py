...
def run_experiment(
    ...
    try:
        ...
        if mode in ("real", "real+mock"):
            evaluator_classes = [GeminiEvaluator, ClaudeEvaluator, OpenAIEvaluator]
            for evaluator_class in evaluator_classes:
                eval_instance = evaluator_class(f"{evaluator_class.__name__}-judge", budget_controller=budget)
                if stateful:
                    evaluators.append(StatefulEvaluator(eval_instance, id=f"stateful-{eval_instance.id}"))
                else:
                    evaluators.append(eval_instance)
    except Exception as e:
        print(f"Could not load real evaluators: {e}")
        pass
    ...
    judge_pool = JudgePool(evaluators, default_n_judges=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_cost_usd", type=float, default=2.0)
